...
function sudoku(t::Array{Int, 2})
	...
	# Another syntax: @variable(m, x[1:n, 1:n, 1:n], Bin)

    ### Constraintes
	# A compléter
	
	@constraint(m, ligne[i in 1:n, k in 1:n], sum(x[i,j,k] for j in 1:n) == 1)
	@constraint(m, colonne[j in 1:n, k in 1:n], sum(x[i,j,k] for i in 1:n) == 1)
	@constraint(m, bloc[di in SetDeb, dj in SetDeb, k in 1:n], sum(x[i,j,k] for i in di:(di+2), j in dj:(dj+2)) == 1)
	@constraint(m, unicite[i in 1:n, j in 1:n], sum(x[i,j,k] for k in 1:n) == 1)

	# Pre remplissage
	@constraint(m, [i in 1:n, j in 1:n, k in 1:n ; t[i,j] == k], x[i,j,k] == 1)
	
	# Pre remplissage (another syntax)
	# for i in 1:n
	# 	for j in 1:n
	# 		for k in 1:n
	# 			if (t[i,j] == k)
	...
	for i in 1:n
		for j in 1:n
			for k in 1:n
				if round(Int, JuMP.value(x[i,j,k])) == 1
					print(k)
					if (t[i,j] == k) print("*")
					else print(" ")
					end
