...
# STOCKAGE
c8{s in STOCKAGES, t in 0..T-1}: NivMin[s] <= niv[s,t] <= NivMax[s];
...
# QUESTION 4
c13{s in STOCKAGES, t in 0..T-1}: alpha[s,t] = midNiv[s,t] * CoeffRedH + (1 - midNiv[s,t]) * CoeffRedB;
c14{s in STOCKAGES, t in 0..T-1}: ((NivMin[s]+NivMax[s])/2) - niv[s,t] >= 10000 * midNiv[s,t];
