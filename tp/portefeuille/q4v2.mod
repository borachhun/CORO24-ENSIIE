...
var alpha{STOCKAGES, 1..T};
var midNiv{STOCKAGES, 1..T} binary;
...
# STOCKAGE
c6{s in STOCKAGES, t in 1..T}: InjMin[s] <= inj[s,t];
c62{s in STOCKAGES, t in 1..T}: inj[s,t] <= alpha[s,t] * InjMax[s];

c7{s in STOCKAGES, t in 1..T}: SouMin[s] <= sou[s,t];
c72{s in STOCKAGES, t in 1..T}: sou[s,t] <= alpha[s,t] * SouMax[s];
...
c13{s in STOCKAGES, t in 1..T}: alpha[s,t] = midNiv[s,t] * CoeffRedH + (1 - midNiv[s,t]) * CoeffRedB;
c14{s in STOCKAGES, t in 1..T}: ((NivMin[s]+NivMax[s])/2) - niv[s,t] >= 10000 * midNiv[s,t];
