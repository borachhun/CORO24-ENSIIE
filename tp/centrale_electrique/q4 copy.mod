...
# c7: sum{t in 1..T} (etat[t,1]+etat[t+1,2])<= NbDemarrage;
# c7: sum{t in 0..T} (etat[t,1] + etat[t+1,2]-etat[t+1,1]) <= 2*NbDemarrage;
c7: sum{t in 0..T} (etat[t,1] - etat[t+1,1]) <= NbDemarrage;
